...
public:
    ...
    int backtrack(vector<string>& arr, string str, int idx){
        unordered_set<char> str_set(str.begin(), str.end()); //get all chars in set
        int len=0;
        if(str_set.size()!=str.size()){  //set size equal to str size if all chars are unique
            return 0;
        }
        len = str.size();
...
public:
    int maxLength(vector<string>& arr) {
        ...
        for(int i=0; i<dp.size(); i++){
           result = max(result, (int)dp[i].count());  //count number of bits set to 1
        }
        return result;
    }
};